...
const Gradient = ({ colors, direction, onClick }) => {
  const handleClick = useCallback(() => {
    onClick(colors);
  }, [colors, onClick]);

  return (
    <Button className="!m-[4px]" onClick={handleClick}>
      <div
...
const BackgroundTab = ({ data, setData }) => {
  ...
  const showColorPicker1 = useCallback(() => {
    setDisplayColorPicker1(true);
  }, []);

  const hideColorPicker1 = useCallback(() => {
    setDisplayColorPicker1(false);
  }, []);

  const showColorPicker2 = useCallback(() => {
    setDisplayColorPicker2(true);
  }, []);

  const hideColorPicker2 = useCallback(() => {
    setDisplayColorPicker2(false);
  }, []);

  const handleColorPicker1Change = useCallback(
    (color) =>
      setData((data) => ({
        ...data,
        background: {
  ...
  const handleColorPicker2Change = useCallback(
    (color) =>
      setData((data) => ({
        ...data,
        background: {
    ...
    <div className="p-7 overflow-x-hidden">
      ...
      {displayColorPicker1 && (
        ...
        >
          ...
          <div style={cover} onClick={hideColorPicker1} />
          <HexColorPicker
            color={data.background.color1}
            onChange={handleColorPicker1Change}
          />
        </div>
      )}
      {displayColorPicker2 && (
        ...
        >
          ...
          <div style={cover} onClick={hideColorPicker2} className="right-0" />
          <HexColorPicker
            color={data.background.color2}
            onChange={handleColorPicker2Change}
          />
        </div>
      )}
